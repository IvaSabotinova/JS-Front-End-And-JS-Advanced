function softUniStudents(input) {
    let courses = {};
    for (const line of input) {
        if (line.includes(':')) {
            let index = line.indexOf(':');
            let courseName = line.slice(0, index);
            let capacity = Number(line.slice(index + 2));
            if (!courses.hasOwnProperty(courseName)) {
                courses[courseName] = {};
                courses[courseName].capacity = 0;
                courses[courseName].students = [];
            }
            courses[courseName].capacity += capacity;
        }
        else {
            let userNameAndCredits = line.split(' ')[0];
            let userEmail = line.split(' ')[3];
            let courseName = line.split(' ')[5];
            let userName = userNameAndCredits.split('[')[0];
            let userCredits_ = userNameAndCredits.split('[')[1];
            let userCredits = userCredits_.slice(0, userCredits_.length - 1);
            if (courses[courseName] && courses[courseName].capacity > 0) {
                courses[courseName].students.push({ userName: userName, email: userEmail, credits: userCredits });
                courses[courseName].capacity -= 1;
            }
        }
    }
    let sortedCourses = Object.entries(courses).sort((a, b) => b[1].students.length - a[1].students.length);

    for (const [name, courseObj] of sortedCourses) {
        console.log(`${name}: ${courses[name].capacity} places left`)
        let sortedCourseObj = courses[name].students.sort((a, b) => b.credits - a.credits);
        for (const { userName, email, credits } of sortedCourseObj) {
            console.log(`--- ${credits}: ${userName}, ${email}`);
        }

    }
}


softUniStudents(['JavaBasics: 2', 'user1[25] with email user1@user.com joins C#Basics', 'C#Advanced: 3', 'JSCore: 4', 'user2[30] with email user2@user.com joins C#Basics', 'user13[50] with email user13@user.com joins JSCore', 'user1[25] with email user1@user.com joins JSCore', 'user8[18] with email user8@user.com joins C#Advanced', 'user6[85] with email user6@user.com joins JSCore', 'JSCore: 2', 'user11[3] with email user11@user.com joins JavaBasics', 'user45[105] with email user45@user.com joins JSCore', 'user007[20] with email user007@user.com joins JSCore', 'user700[29] with email user700@user.com joins JSCore', 'user900[88] with email user900@user.com joins JSCore']);

softUniStudents(['JavaBasics: 15',
'user1[26] with email user1@user.com joins JavaBasics',
'user2[36] with email user11@user.com joins JavaBasics',
'JavaBasics: 5',
'C#Advanced: 5',
'user1[26] with email user1@user.com joins C#Advanced',
'user2[36] with email user11@user.com joins C#Advanced',
'user3[6] with email user3@user.com joins C#Advanced',
'C#Advanced: 1',
'JSCore: 8',
'user23[62] with email user23@user.com joins JSCore']);